...
// ── SEKIRITE: header HTTP debaz (CSP, no-sniff, frame-deny, etc.) ──
app.use(helmet({
  contentSecurityPolicy: false, // sit la chaje script/style soti plizyè CDN — desaktive pou evite blokaj; aktive l apre ou fin lis tout domèn ou itilize
  crossOriginEmbedderPolicy: false,
}));
app.set('trust proxy', 1); // nesesè pou rate-limit lè ou dèyè Render/Railway/Vercel

// ── RATE LIMIT: pwoteje /api/* kont abi/spam/bot ──
const apiLimiter = rateLimit({
  windowMs: 15 * 60 * 1000, // 15 minit
  max: 200, // 200 demand pa IP pa fenèt
  standardHeaders: true,
  legacyHeaders: false,
  message: { data: null, error: 'Twòp demand. Tann kèk minit epi eseye ankò.' },
});
app.use('/api/', apiLimiter);

app.use(express.json());

// ── ROUTE: /config.js — bay kliyan konfig Supabase piblik la (san hardcode) ──
app.get('/config.js', (req, res) => {
  const url = process.env.SUPABASE_URL || '';
  const anonKey = process.env.SUPABASE_ANON_KEY || '';
  const configured = Boolean(url && anonKey && !url.includes('YOUR_PROJECT'));
  res.set('Content-Type', 'application/javascript');
  res.send(`window.__SUPABASE_CONFIG__ = ${JSON.stringify({ url, anonKey, configured })};`);
});
