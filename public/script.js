...
// Listings done — ranpli soti nan /api/listings (Supabase oswa demo,
// kontwole pa server a). Kòmanse vid, ranpli pa fetchInitialListings().
let MOCK_LISTINGS = [];

// Si server a te enjekte done anons aktyèl la dirèkteman nan paj la
// (SSR pou /annonce/:id-slug), itilize l tousuit san atann fetch.
if (window.__INITIAL_LISTING__) {
 MOCK_LISTINGS.push(window.__INITIAL_LISTING__);
}
...
async function fetchInitialListings() {
 try {
  ...
  // Fusyone san double (si __INITIAL_LISTING__ te deja la)
  items.forEach(item => {
   if (!MOCK_LISTINGS.find(l => String(l.id) === String(item.id))) {
    MOCK_LISTINGS.push(item);
   }
  });
 } catch (err) {
  console.error('Pa t ka chaje anons yo:', err);
 }
}
...
function navigate(page, data = {}, skipPushState = false) {
 ...
 // Chanje URL navigatè a san rechaje paj la (sa pèmèt pataje lyen
 // ak retounen ak bouton "back" navigatè a mache kòrèkteman)
 if (!skipPushState) {
  let url = '/';
  if (page === 'listing' && data.id) {
   const item = MOCK_LISTINGS.find(l => String(l.id) === String(data.id));
   const slug = item ? slugify(item.title) : 'anons';
   url = `/annonce/${data.id}-${slug}`;
  } else if (page === 'home') {
   url = '/';
  }
...
function renderListingDetail(id) {
 const item = MOCK_LISTINGS.find(l => String(l.id) === String(id)) || MOCK_LISTINGS[0];
 if (!item) {
  const container = $('#page-listing');
  if (container) container.innerHTML = `<div class="empty-state" style="padding:60px 16px;text-align:center">
   <div class="empty-icon"></div>
   <h3>Anons sa pa disponib</h3>
   <p>Anons sa ka te efase oswa li pa egziste.</p>
   <button class="btn btn-primary" onclick="navigate('home')" style="margin-top:16px">Retounen Akèy</button>
  </div>`;
  return;
 }
...
function sharelisting(id) {
 const item = APP.currentListing || MOCK_LISTINGS.find(l => String(l.id) === String(id));
 const slug = item ? slugify(item.title) : 'anons';
 const url = `${window.location.origin}/annonce/${id}-${slug}`;
 if (navigator.share) {
...
function reportListing(id) {
 toast('Rapò voye bay ekip nou an. Mèsi!', 'info');
}
...
async function handleLogin(e) {
 ...
 // Demo auth (replace with Supabase)
 setTimeout(() => {
 APP.currentUser = { id: 'demo123', email, user_metadata: { full_name: 'Mwen Menm' } };
 APP.currentProfile = { id: 'demo123', username: email.split('@')[0], full_name: 'Mwen Menm', is_admin: email.includes('admin') };
 updateAuthUI();
 closeAuthModal();
...
async function handleRegister(e) {
 ...
 setTimeout(() => {
 APP.currentUser = { id: 'demo123', email, user_metadata: { full_name: name } };
 APP.currentProfile = { id: 'demo123', username: email.split('@')[0], full_name: name };
 updateAuthUI();
 closeAuthModal();
 toast('Kont kreye avèk siksè! Byenveni ', 'success');
 btn.disabled = false;
...
document.addEventListener('DOMContentLoaded', () => {
 ...
 // Bouton back/forward navigatè a
 window.addEventListener('popstate', (e) => {
  if (e.state && e.state.page) {
   navigate(e.state.page, e.state.data || {}, true);
  } else {
   initFromUrl(true);
  }
 });

 // Init paj la selon URL aktyèl (annonce/id-slug oswa akèy)
 initFromUrl();
 updateAuthUI();
});

// ── INIT SELON URL ───────────────────────────────────────────
async function initFromUrl(skipPushState = false) {
 await fetchInitialListings();

 const path = window.location.pathname;
 const annonceMatch = path.match(/^\/annonce\/([^/]+)/);

 if (annonceMatch) {
  const idSlug = annonceMatch[1];
  const id = idSlug.split('-')[0];
  navigate('listing', { id }, true); // URL la deja kòrèk, pa pushState
  return;
 }

 // Sipòte ansyen fòma ?listing=ID tou (konpatibilite ak vye lyen pataje)
 const params = new URLSearchParams(window.location.search);
 const listingParam = params.get('listing');
 if (listingParam) {
  navigate('listing', { id: listingParam }, true);
  return;
 }
